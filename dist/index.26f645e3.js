...
const initHeaderObserverTop = function(section, headerEl) {
    const headerExtend = function(entries) {
        if (entries[0].isIntersecting) headerEl.classList.remove(headerSquashedClass);
    };
...
const initHeaderObserverBottom = function(section, headerEl) {
    const headerSquash = function(entries) {
        if (entries[0].isIntersecting) headerEl.classList.add(headerSquashedClass);
    };
...
const displayProjectCard = function(target, cardPosition, cardsWrapper) {
    ...
    if (cardPosition === 'left') cardsWrapper.style.transform = 'translate(' + cardWidth + 'px, 0)';
    if (cardPosition === 'center') cardsWrapper.style.transform = 'translate(0, 0)';
    if (cardPosition === 'right') cardsWrapper.style.transform = 'translate(-' + cardWidth + 'px, 0)';
};
...
window.addEventListener('DOMContentLoaded', ()=>{
    ...
    const projectCardCenterEl = document.querySelector('.js-project-card-center');
    const projectCardRightEl = document.querySelector('.js-project-card-right');
    const projectCardsWrapperEl = document.querySelector('.js-projects-cards-wrapper');
